...
export async function POST(request: NextRequest) {
  try {
    ...
    const sheetTitle = `NDA_${fullName}_${now.getFullYear()}-${(now.getMonth() + 1).toString().padStart(2, '0')}-${now.getDate().toString().padStart(2, '0')}`
    ...
    // Содержимое NDA разбитое на строки для Google Sheets
    const ndaRows = [
    ...
    // Пытаемся переместить таблицу в папку NDA
    try {
      ...
      console.log('Spreadsheet moved to NDA folder successfully')
    } catch (moveError: any) {
      console.warn('Could not move to NDA folder, spreadsheet stays in root:', moveError.message)
    }
    ...
    const { error: insertError } = await supabase
      ...
      .insert([{
        ...
        document_url: `https://docs.google.com/spreadsheets/d/${spreadsheetId}`,
    ...
    return NextResponse.json({
      ...
      documentUrl: `https://docs.google.com/spreadsheets/d/${spreadsheetId}`
    })
