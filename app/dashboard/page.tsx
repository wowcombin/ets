        ...
        {activeTab === 'employees' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="p-6">
                  <div className="space-y-2">
                    {activeEmployees.map(emp => (
                      <div key={emp.id} className="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg">
                        <div className="flex items-center gap-3">
                          <div className={`w-2 h-2 rounded-full ${emp.is_manager ? 'bg-blue-400' : 'bg-green-400'}`} />
                          <span className="font-medium">{emp.username}</span>
                        </div>
                        <span className="text-sm text-gray-400">
                          {emp.is_manager ? `Менеджер (${emp.profit_percentage}%)` : 'Сотрудник'}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              {/* Fired Employees */}
              {data?.firedEmployees && data.firedEmployees.length > 0 && (
                <div className="bg-gray-800 border border-gray-700 rounded-lg">
                  <div className="p-6 border-b border-gray-700">
                    <h2 className="text-xl font-bold flex items-center gap-2">
                      <UserX className="w-5 h-5 text-red-400" />
                      Уволенные ({data.firedEmployees.length})
                    </h2>
                  </div>
                  <div className="p-6">
                    <div className="space-y-2">
                      {data.firedEmployees.map(emp => (
                        <div key={emp.id} className="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg opacity-60">
                          <div className="flex items-center gap-3">
                            <div className="w-2 h-2 rounded-full bg-red-400" />
                            <span className="font-medium line-through">{emp.username}</span>
                          </div>
                          <span className="text-sm text-gray-500">Уволен</span>
                        </div>
                      ))}
                    </div>
                  </div>
                      ...
                      <tbody>
                        {data.employeeStats.map((stat) => (
                          <tr key={stat.id} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                            ...
                            <td className={`py-3 px-4 text-right font-bold ${
                              stat.totalGross >= 0 ? 'text-green-400' : 'text-red-400'
                            }`}>
                              ${stat.totalGross.toFixed(2)}
                            </td>
                            <td className="py-3 px-4 text-right">{stat.casinos.length}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div className="p-6">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {data.casinoStats.map((stat) => (
                        <tr key={stat.name} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                          ...
                          <td className="py-3 px-4 text-right">{stat.employees.length}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        {activeTab === 'cards' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-gray-400">Использовано</span>
                  <Activity className="w-5 h-5 text-red-400" />
                </div>
                <div className="text-3xl font-bold text-red-400">{data?.stats?.usedCardCount || 0}</div>
              </div>
              
              <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-gray-400">Доступно</span>
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-lg">
              <div className="p-6 border-b border-gray-700">
                <h2 className="text-xl font-bold">Карты по статусу</h2>
              </div>
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Used Cards */}
                  <div>
                    <h3 className="text-lg font-semibold mb-3 text-red-400">Использованные карты</h3>
                    <div className="space-y-2 max-h-96 overflow-y-auto">
                      {data?.cards?.filter(c => c.status === 'used').map(card => (
                        <div key={card.id} className="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg">
                          <div>
                            <div className="font-mono text-sm">{card.card_number}</div>
                            <div className="text-xs text-gray-400">{card.casino_name || 'Unknown'}</div>
                          </div>
                  ...
                  {/* Available Cards */}
                  <div>
                    <h3 className="text-lg font-semibold mb-3 text-green-400">Доступные карты</h3>
                    <div className="space-y-2 max-h-96 overflow-y-auto">
                      {data?.cards?.filter(c => c.status === 'available').slice(0, 10).map(card => (
                        <div key={card.id} className="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg">
                          <div className="font-mono text-sm">{card.card_number}</div>
                    ...
                    {data?.cards && data.cards.filter(c => c.status === 'available').length > 10 && (
                      <p className="text-sm text-gray-400 mt-3">
                        И еще {data.cards.filter(c => c.status === 'available').length - 10} карт...
                      </p>
                    )}
                  </div>
                </div>
